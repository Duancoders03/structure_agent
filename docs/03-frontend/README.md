...
- **Hệ thống Bộ lọc (Filtering System):**
    - **Cấu trúc Flex-Flow:** Sử dụng `flex-wrap` để tự động điều chỉnh bố cục bộ lọc trên các kích thước màn hình khác nhau.
    - **Search Component (Robust Flex Layout):** Thay vì sử dụng định vị tuyệt đối, ô tìm kiếm được xây dựng trên nền tảng Flexbox để đảm bảo biểu tượng và văn bản không bao giờ bị chồng lấn.
    - **Interactive Feedback:** Sử dụng `focus-within` trên container để thay đổi trạng thái (border, shadow) khi người dùng tương tác với ô nhập liệu bên trong.
...
- **Data Tables:** Sử dụng nền trắng, bo góc lớn (`2rem`), phân tách bằng đường kẻ mảnh `slate-50`. Hiệu ứng `group-hover` được áp dụng để làm nổi bật dòng dữ liệu khi di chuột qua.
- **Management Forms (Biểu mẫu quản lý):**
    - **Layout:** Sử dụng bố cục 2 cột (Main Content & Sidebar Action) trên màn hình lớn để tối ưu không gian nhập liệu.
    - **Sectioning:** Chia form thành các khối logic (Thông tin cơ bản, Phân loại, Media) giúp người dùng không bị ngợp thông tin.
    - **Interactive States:** Các trường nhập liệu có hiệu ứng `focus:border-emerald-500` và `focus:ring-4` tạo cảm giác phản hồi tức thì.
    - **CRUD Interactions:** Tích hợp các nút hành động (Edit/Delete) trực tiếp trên dòng dữ liệu, có hộp thoại xác nhận khi thực hiện các hành động không thể hoàn tác.
